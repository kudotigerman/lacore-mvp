...
export default function LandingPage({ content, slug }: Props) {
  ...
  return (
    <div style={{ background: theme.bgPrimary, color: "#FAFAFA", fontFamily: "Inter, sans-serif" }}>
      <style>{`
      ...
      >
        <div className="container" style={{ display: "flex", height: 80, alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ fontFamily: "Plus Jakarta Sans", fontWeight: 800, fontSize: 20 }}>{content.brand}</div>
          <div className="hide-mobile" style={{ display: "flex", gap: 28, alignItems: "center" }}>
            ...
            <a href="#contact-form" style={{ background: theme.accent, color: "#fff", padding: "10px 20px", borderRadius: 10 }}>{content.ctaPrimary}</a>
          </div>
        </div>
      </nav>

      <section data-aos="fade-up" style={{ minHeight: "100vh", display: "flex", alignItems: "center", ...heroBg }}>
        {theme.heroStyle === "gradient-blob" ? (
          <>
            <div style={{ position: "absolute", width: 600, height: 600, borderRadius: "50%", background: theme.accent, filter: "blur(120px)", opacity: 0.15, top: -100, left: -100 }} />
            <div style={{ position: "absolute", width: 400, height: 400, borderRadius: "50%", background: theme.accentLight, filter: "blur(100px)", opacity: 0.1, bottom: 0, right: 0 }} />
          </>
        ) : null}
        <div className="container" style={{ position: "relative", zIndex: 1, paddingTop: 60 }}>
          <div data-aos="fade-up" style={{ display: "inline-flex", gap: 8, border: "1px solid rgba(255,255,255,0.12)", borderRadius: 999, padding: "8px 14px", marginBottom: 24 }}>
            ...
            <span style={{ color: "#A1A1AA", fontSize: 12, letterSpacing: "0.08em" }}>{content.badge}</span>
          </div>
          ...
          <p data-aos="fade-up" style={{ color: "#A1A1AA", fontSize: 20, maxWidth: 700, lineHeight: 1.7 }}>{content.subheadline}</p>
          <div data-aos="fade-up" style={{ display: "flex", gap: 14, flexWrap: "wrap", marginTop: 30 }}>
            <a href="#contact-form" style={{ background: theme.accent, color: "#fff", padding: "16px 28px", borderRadius: 12, fontWeight: 700, display: "inline-flex", gap: 10, alignItems: "center" }}>{content.ctaPrimary}<ArrowRight color="#fff" /></a>
            <a href="#process" style={{ background: "rgba(255,255,255,0.06)", color: "#FAFAFA", border: "1px solid rgba(255,255,255,0.1)", padding: "16px 28px", borderRadius: 12 }}>{content.ctaSecondary}</a>
          </div>
        ...
        <div className="container grid3">
          {content.stats.slice(0, 3).map((s, i) => (
            <div data-aos="fade-up" data-aos-delay={i * 100} key={s.label} style={{ textAlign: "center" }}>
              <div style={{ color: theme.accent, fontFamily: "Plus Jakarta Sans", fontWeight: 900, fontSize: 52 }}>{s.number}</div>
      ...
      <section id="benefits" data-aos="fade-up">
        <div className="container">
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            <div style={{ color: theme.accent, fontSize: 12, letterSpacing: "0.1em", fontWeight: 700 }}>THE PROBLEM</div>
            <h2 style={{ fontFamily: "Plus Jakarta Sans", fontSize: "clamp(32px,5vw,52px)", marginTop: 14 }}>{content.problemHeadline}</h2>
          </div>
          <div className="grid3">
      ...
      <section data-aos="fade-up" style={{ background: theme.bgSecondary }}>
        <div className="container">
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            <div style={{ color: theme.accent, fontSize: 12, letterSpacing: "0.1em", fontWeight: 700 }}>THE SOLUTION</div>
            <h2 style={{ fontFamily: "Plus Jakarta Sans", fontSize: "clamp(32px,5vw,52px)", marginTop: 14 }}>{content.solutionHeadline}</h2>
          </div>
          <div className="grid3">
            {content.features.slice(0, 3).map((f, i) => {
              ...
              return (
                <div data-aos="fade-up" data-aos-delay={i * 100} key={f.title} style={{ background: theme.bgPrimary, border: "1px solid #1C1C22", borderRadius: 16, padding: 28 }}>
                  ...
                  <p style={{ margin: 0, color: "#A1A1AA", lineHeight: 1.65 }}>{f.desc}</p>
                </div>
              );
            })}
          </div>
      ...
      <section id="process" data-aos="fade-up">
        <div className="container" style={{ maxWidth: 860 }}>
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            <div style={{ color: theme.accent, fontSize: 12, letterSpacing: "0.1em", fontWeight: 700 }}>HOW IT WORKS</div>
            <h2 style={{ fontFamily: "Plus Jakarta Sans", fontSize: "clamp(32px,5vw,52px)", marginTop: 14 }}>{content.processHeadline}</h2>
          </div>
          {content.steps.slice(0, 3).map((s, i) => (
            <div data-aos="fade-up" data-aos-delay={i * 100} key={s.title} style={{ display: "flex", gap: 18, paddingBottom: i < 2 ? 28 : 0 }}>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                <div style={{ width: 46, height: 46, borderRadius: "50%", background: theme.accent, color: "#fff", display: "grid", placeItems: "center", fontWeight: 800 }}>{i + 1}</div>
                {i < 2 ? <div style={{ width: 2, flex: 1, marginTop: 6, background: `linear-gradient(to bottom, ${theme.accent}, transparent)` }} /> : null}
              </div>
              <div style={{ paddingTop: 6 }}>
                <h3 style={{ margin: "0 0 8px" }}>{s.title}</h3>
                <p style={{ margin: 0, color: "#A1A1AA" }}>{s.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section id="testimonials" data-aos="fade-up" style={{ background: theme.bgSecondary }}>
        <div className="container">
          <div style={{ textAlign: "center", marginBottom: 56 }}>
            <div style={{ color: theme.accent, fontSize: 12, letterSpacing: "0.1em", fontWeight: 700 }}>RESULTS</div>
            <h2 style={{ fontFamily: "Plus Jakarta Sans", fontSize: "clamp(32px,5vw,52px)", marginTop: 14 }}>{content.testimonialsHeadline}</h2>
          </div>
          <div className="grid3">
      ...
      <section data-aos="fade-up" style={{ background: `linear-gradient(135deg, ${theme.accent}1f 0%, ${theme.bgPrimary} 60%)` }}>
        <div className="container" style={{ textAlign: "center", maxWidth: 860 }}>
          <h2 style={{ fontFamily: "Plus Jakarta Sans", fontSize: "clamp(36px,6vw,64px)", margin: "0 0 18px" }}>{content.ctaHeadline}</h2>
          <p style={{ color: "#A1A1AA", fontSize: 18, margin: "0 0 28px" }}>{content.ctaSubtext}</p>
          <a href="#contact-form" style={{ display: "inline-flex", gap: 10, alignItems: "center", background: theme.accent, color: "#fff", padding: "18px 36px", borderRadius: 12, fontWeight: 800 }}>{content.ctaButton}<ArrowRight color="#fff" /></a>
        </div>
      </section>
        ...
        <div className="container" style={{ maxWidth: 720 }}>
          ...
          <form onSubmit={handleSubmit} style={{ background: theme.bgSecondary, border: "1px solid #1C1C22", borderRadius: 18, padding: 30 }}>
            <div style={{ display: "grid", gap: 16 }}>
              ...
              <button disabled={loading} type="submit" style={{ width: "100%", border: "none", cursor: "pointer", borderRadius: 10, padding: "15px 16px", color: "#fff", background: theme.accent, fontWeight: 700 }}>
