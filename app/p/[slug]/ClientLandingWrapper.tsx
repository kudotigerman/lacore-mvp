...
import { Component, type CSSProperties, type ErrorInfo, type ReactNode, useCallback, useEffect, useMemo, useRef, useState } from "react";
...
export type ClientLandingWrapperProps = {
  slug: string;
  initialHtml: string;
  initialJsx: string;
  initialJsonContent: LandingContent | null;
  initialLandingStyle: string;
};
...
function PublicLandingPageContent({
  ...
  const [jsxContent, setJsxContent] = useState(initialJsx);
  const [jsonContent, setJsonContent] = useState<LandingContent | null>(initialJsonContent);
  const [landingStyle, setLandingStyle] = useState<string>(initialLandingStyle);
...
export default function ClientLandingWrapper(props: ClientLandingWrapperProps) {
  return (
    <PublicLandingPageErrorBoundary>
      <PublicLandingPageContent {...props} />
    </PublicLandingPageErrorBoundary>
  );
}
